...
def dictListCmp(dl1, dl2):
	return cmp(dl2[1], dl1[1])
...
def printDictListForKorKey(dl):
	if len(dl) == 0:
		print '[]'
	else:
		print '[',
		for tempDl in dl:
			print '(\'' + tempDl[0] + '\',' + str(tempDl[1]) +')',
		print ']'
...
class OrderedWebWordsFrequency(WebWordsFrequency):
	def getWordsFrequency(self):
		...
		for key, value in totalDict.items():
			totalDictList.append((key, value))
			totalDictList.sort(dictListCmp)

		return totalDictList
		

if __name__ == "__main__":
	w4 = OrderedWebWordsFrequency('http://www.daum.net', 'http://www.naver.com', 'http://www.google.co.kr')
	printDictListForKorKey(w4.getWordsFrequency())
